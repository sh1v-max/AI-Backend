...
- **Next up:** Phase 4's *required* steps are done (4.1, 4.2, 4.2F). Remaining: optional **Step 4.3** (`POST /quiz/check`) + **4.3F** (answer-checking UI) — closes the loop but isn't required for the milestone. After that, Phase 5 (tie it together, write it up) or Phase 6 (background jobs). See [PROGRESS.md](PROGRESS.md) and the walkthrough.
...
- **Git:** the checked-out branch is `zod-branch` (last commit when written: `5d1645a "step 4.2 done, build quiz endpoint"`, 2026-09-23). `Streaming-branch` was merged into `main` through PR #1; Step 3.3, all of Step 4.1 and all of Step 4.2 (backend) are committed. **Uncommitted when written:** Step 4.2F's frontend code (`frontend/src/types/quiz.ts`, `api/quiz.ts`, `hooks/useQuiz.ts`, `components/quiz/QuizModal.tsx`, the `App.tsx`/`ChatView.tsx`/`App.css` edits) and the latest docs updates. Shiv commits everything himself — run `git status` to see what's pending.
...
frontend/src/
  ...
  components/  common/, sidebar/, chat/, documents/, quiz/ (QuizModal.tsx — overlay, radio buttons per question, no grading yet)
