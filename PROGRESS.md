...
| 08 | ✅ | Structured output (Zod) | [topics/08-structured-output-zod](topics/08-structured-output-zod/NOTES.md) — Step 4.1 (Zod quiz schema, `sampleChunks`, `npm run step4` comparing plain vs structured-output mode, bad-reply gallery) and **Step 4.2 `POST /quiz`** (validate → retry once → shuffle options, rejects `'all'`) both done. **Next:** Step 4.2F (Generate-quiz button + UI), then optional 4.3 | `advanced/01` Prompt engineering |
