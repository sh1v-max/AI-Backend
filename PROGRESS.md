...
| 06 | ✅ | Conversation memory | [topics/06-conversation-memory](topics/06-conversation-memory/NOTES.md) — `chat_messages` table + repository done; `/chat` rewritten as a 7-step pipeline (embed → search → load history → save user msg → build layered prompt → generate → save assistant reply); frontend generates one `sessionId` per document, reused across messages; sources display added to chat UI. Verified end to end in Neon (real session, 8 rows, correct history resolution on a vague follow-up). **Previously-known gap, now fixed:** added `GET /sessions` + `GET /sessions/:sessionId/messages`, and the frontend was redesigned around a ChatGPT-style history sidebar that persists the active `sessionId` to `localStorage` and rehydrates the transcript on reload — a refresh no longer orphans the conversation. | `advanced/03` Token counting & cost |
| 07 | ⬜ | Streaming (SSE) | [topics/07-streaming-sse](topics/07-streaming-sse/NOTES.md) — after this, do Step 3.3 in [ai-backend-roadmap.md](ai-backend-roadmap.md) (multi-document chat: search all PDFs by default, single-PDF as an override) before moving to topic 08 | — |
