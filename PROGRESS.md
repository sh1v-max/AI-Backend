...
| 06 | ✅ | Conversation memory | [topics/06-conversation-memory](topics/06-conversation-memory/NOTES.md) — `chat_messages` table + repository done; `/chat` rewritten as a 7-step pipeline (embed → search → load history → save user msg → build layered prompt → generate → save assistant reply); frontend generates one `sessionId` per document, reused across messages; sources display added to chat UI. Verified end to end in Neon (real session, 8 rows, correct history resolution on a vague follow-up). **Known gap, not yet fixed:** `sessionId` + visible chat history only live in React state — refreshing the tab starts a brand-new session, so the old conversation is orphaned (still in Postgres, just unreachable from the UI). Fix later: persist `sessionsByDocument` to `localStorage` + add a history-fetch endpoint so a refresh can rehydrate the same session. | `advanced/03` Token counting & cost |
