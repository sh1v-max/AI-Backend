...
| 08 | 🟡 | Structured output (Zod) | [topics/08-structured-output-zod](topics/08-structured-output-zod/NOTES.md) — **in progress:** Step 4.1 done (Zod quiz schema, `sampleChunks`, `npm run step4` comparing plain vs structured-output mode, bad-reply gallery). **Next:** Step 4.2 `POST /quiz` (validate → retry once, shuffle options) | `advanced/01` Prompt engineering |
