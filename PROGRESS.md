...
| 07 | ✅ | Streaming (SSE) | [topics/07-streaming-sse](topics/07-streaming-sse/NOTES.md) — Steps 3.1, 3.2, 3.2F done: throwaway `GET /tick` for the SSE mechanics, then `GET /chat-stream` (shared `buildChatPrompt()`, `streamAnswer()` async generator over Gemini's `streamGenerateContent?alt=sse`, `meta` / data / `done` / `error` events, `withErrorHandling()` for readable failures) and a frontend `EventSource` wrapper so replies type out live. The throwaway `/tick` has been deleted. **Next:** do Step 3.3 in [ai-backend-roadmap.md](ai-backend-roadmap.md) (multi-document chat: search all PDFs by default, single-PDF as an override) before moving to topic 08 | — |
