...
| 08 | ✅ | Structured output (Zod) | [topics/08-structured-output-zod](topics/08-structured-output-zod/NOTES.md) — all of Phase 4 done: 4.1 (Zod quiz schema, `npm run step4`), 4.2 (`POST /quiz` — validate → retry once → shuffle options), 4.2F (Generate-quiz button + modal), and **4.3/4.3F** (grading — done client-side, no `/quiz/check` endpoint; see the walkthrough for why). Phase 4's milestone is reached: upload → chat → quiz → check, all in the browser. **Next:** Phase 5 (tie it together, write it up), or Phase 6 (background jobs) | `advanced/01` Prompt engineering |
